...
Updates were necessary due to (1) changed pupil-lab data formats and to (2) the slightly different use case.
Current version also extracts pupil diameter and transforms timestamps into unix time for synchronization with
further devices.
...
python3 pl_preprocessing.py --input_dir "INPUTDIR" --output_dir "OUTPUTDIR" --sid "SID"

Parameters:
input_dir:      str, path to data dir (subject directory created by pupil)
output_dir:     str, path for output dir
sid:            str, session ID for data set (e.g. '006')
...
sys.path.append('/home/adamb/PycharmProjects/pupilCore/preprocessing_code/1_to_common_format/pupillabs/')
import file_methods as fm



def preprocess_data(input_dir, output_root, sid):
    """
    ...
    sid:             str, session ID for data set
    ...
    output_dir = join(output_root, sid)
...
if __name__ == '__main__':
    ...
    parser.add_argument('input_dir', help='path to the raw pupil labs recording dir for subject')
    parser.add_argument('output_dir', help='output directory root')
    parser.add_argument('sid', help='session ID')
    ...
    if not os.path.isdir(args.input_dir):
        ...
        preprocess_data(args.input_dir, args.output_dir, args.sid)
